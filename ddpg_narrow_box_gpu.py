...
class ddpg_agent(object):
	...
	def create_critic_network(self,inputdims,actiondims):
		...
		with tf.device('/device:GPU:0'):
			...
			den0 = c.add(d(inputdims, 1200))

			den1 = c.add(d(1200 + actiondims,800))
			den2 = c.add(d(800,400))
			den2a = c.add(d(400,200))
	...
	def play_one_ep(self, env, noise_level, frameskip, max_steps = 50000):
		...
		while True and steps <= max_steps:
			...
			for _ in range(frameskip):
				...
				lr, done = reward_shaping(observation_d, reward)
				learning_reward += lr
				if done:
					break
