...
class ddpg_agent(object):
	...
	def create_actor_network(self,inputdims,outputdims):
		...
		with tf.device('/device:GPU:0'):
			...
			c.add(Dense(2000,outputdims,stddev=1))
	...
	def create_critic_network(self,inputdims,actiondims):
		...
		with tf.device('/device:GPU:0'):
			...
			den0 = c.add(d(inputdims, 8000))

			den1 = c.add(d(8000 + actiondims,6000))
			den2 = c.add(d(6000,2000))
			den2a = c.add(d(2000,1000))
			den3 = c.add(Dense(1000,1,stddev=1))
	...
	def play_one_ep(self, env, noise_level, frameskip, max_steps = 1000):
		...
		while True and steps <= max_steps:
			...
			for _ in range(frameskip):
				...
				lr, done = reward_shaping(action, observation_d, reward, done)
				learning_reward += lr
		...
		print('episode done in {} steps in {:.2f} sec, {:.4f} sec/step, got reward :{:.2f}, learning reward :{:.2f}'.format(
		steps,totaltime,totaltime/steps,total_reward, ep_learning_reward
		))
	...
	def train(self, batch_size = 64, min_memory_size = 1000):
	...
	def save_weights(self, dir_prefix = 'training_saves/weights/'):
		...
		for name in networks:
			...
			network.save_weights(dir_prefix + 'ddpg_'+name+'_gpu.npz')

	def load_weights(self, dir_prefix = 'training_saves/weights/'):
		networks = ['actor','critic','actor_target','critic_target']
		for name in networks:
			network = getattr(self,name)
			network.load_weights(dir_prefix + 'ddpg_'+name+'_gpu.npz')
...
if __name__=='__main__':
	...
	print('Creating Agent, using muscle activation penalty')
	...
	def train(eps, save_interval = 100, frameskip = 4):
	...
	def save():
		...
		agent.save_weights()
		agent.rpm.save('rpm_gpu.pickle')

		with open('reward_history_gpu.p', 'wb') as f:
